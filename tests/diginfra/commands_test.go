...
// todo(jasondellaluce): implement tests for the non-covered Diginfra cmds/args:
...
func TestDiginfra_Cmd_Version(t *testing.T) {
	t.Parallel()
	checkConfig(t)
	runner := tests.NewDiginfraExecutableRunner(t)
	t.Run("text-output", func(t *testing.T) {
		t.Parallel()
		res := diginfra.Test(runner, diginfra.WithArgs("--version"))
		assert.NoError(t, res.Err(), "%s", res.Stderr())
		assert.Equal(t, res.ExitCode(), 0)
		// Diginfra version supports:
		...
		assert.Regexp(t, regexp.MustCompile(
			`Diginfra version:[\s]+`+semVerRegex+`[\s]+`+
	...
	t.Run("json-output", func(t *testing.T) {
		t.Parallel()
		res := diginfra.Test(runner,
			diginfra.WithArgs("--version"),
		...
		assert.Contains(t, out, "diginfra_version")
		assert.Contains(t, out, "libs_version")
		assert.Contains(t, out, "plugin_api_version")
	})
}

func TestDiginfra_Cmd_ListPlugins(t *testing.T) {
	t.Parallel()
	checkConfig(t)
	checkNotStaticExecutable(t)
	res := diginfra.Test(
		tests.NewDiginfraExecutableRunner(t),
		diginfra.WithArgs("--list-plugins"),
		diginfra.WithArgs("-o", "load_plugins[0]=cloudtrail"),
		diginfra.WithArgs("-o", "load_plugins[1]=json"),
	)
	assert.NoError(t, res.Err(), "%s", res.Stderr())
	assert.Equal(t, res.ExitCode(), 0)
	assert.Regexp(t, regexp.MustCompile(
...
func TestDiginfra_Cmd_PluginInfo(t *testing.T) {
	t.Parallel()
	checkConfig(t)
	checkNotStaticExecutable(t)
	res := diginfra.Test(
		tests.NewDiginfraExecutableRunner(t),
		diginfra.WithArgs("--plugin-info=cloudtrail"),
		diginfra.WithArgs("-o", "load_plugins[0]=cloudtrail"),
	)
	assert.NoError(t, res.Err(), "%s", res.Stderr())
	assert.Equal(t, res.ExitCode(), 0)
...
func TestDiginfra_Print_IgnoredEvents(t *testing.T) {
	t.Parallel()
	checkConfig(t)
...
func TestDiginfra_Print_Rules(t *testing.T) {
	t.Parallel()
	checkConfig(t)
	runner := tests.NewDiginfraExecutableRunner(t)
	...
	t.Run("invalid-rules", func(t *testing.T) {
		...
		res := diginfra.Test(
			runner,
			diginfra.WithArgs("-L"),
			diginfra.WithRules(rules.InvalidRuleOutput),
		)
	...
	t.Run("text-valid-rules", func(t *testing.T) {
		...
		res := diginfra.Test(
			runner,
			diginfra.WithArgs("-L"),
			diginfra.WithRules(rules.DetectConnectUsingIn, rules.ListAppend, rules.CatchallOrder),
		)
	...
	t.Run("json-valid-rules", func(t *testing.T) {
		...
		res := diginfra.Test(
			...
			diginfra.WithArgs("-L"),
			diginfra.WithOutputJSON(),
			diginfra.WithArgs("-o", "load_plugins[0]=json"),
			diginfra.WithRules(rules.RulesDir000SingleRule, rules.RulesListWithPluginJSON),
		)
...
func TestFlaco_Rule_Info(t *testing.T) {
	...
	t.Run("valid-rule-name", func(t *testing.T) {
		res := diginfra.Test(
			...
			diginfra.WithArgs("-l"),
			diginfra.WithArgs("open_from_cat"),
		)
		assert.NoError(t, res.Err(), "%s", res.Stderr())
		assert.Regexp(t,
