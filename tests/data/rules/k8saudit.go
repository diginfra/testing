...
var K8SAuditRules = run.NewLocalFileAccessor(
	...
	"/etc/diginfra/k8s_audit_rules.yaml",
)
...
- rule: Create Privileged Pod
  ...
  condition: kevt and pod and kcreate and ka.req.container.privileged=true and not ka.req.container.image.repository in (diginfra_privileged_images)
...
- rule: Create Sensitive Mount Pod
  ...
  condition: kevt and pod and kcreate and sensitive_vol_mount and not ka.req.container.image.repository in (diginfra_sensitive_mount_images)
...
- rule: Create HostNetwork Pod
  ...
  condition: kevt and pod and kcreate and ka.req.container.host_network=true and not ka.req.container.image.repository in (diginfra_hostnetwork_images)
