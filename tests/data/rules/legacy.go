...
// LegacyDiginfraRules_v1_0_1 is a local copy of:
// https://github.com/diginfra/rules/blob/diginfra-rules-1.0.1/rules/diginfra_rules.yaml
//
// It is intended to be used with legacy tests.
// Please note that legacy tests are designed to test Diginfra features, not the ruleset.
var LegacyDiginfraRules_v1_0_1 = run.NewStringFileAccessor(
	"diginfra_rules.yaml",
	`
    #
    # Copyright (C) 2024 The Diginfra Authors.
    ...
    # Starting with version 8, the Diginfra engine supports exceptions.
    # However the Diginfra rules file does not use them by default.
    ...
    # Information about rules tags and fields can be found here: https://diginfra.org/docs/rules/#tags-for-current-diginfra-ruleset
    ...
    # truncated at the diginfra-libs level.
    ...
    # are allowed.
    #
    # In the main diginfra rules file, there isn't any way to know the
    ...
    # ******************************************************************************
    # * "Directory traversal monitored file read" requires DIGINFRA_ENGINE_VERSION 13 *
    # ******************************************************************************
    ...
    - macro: write_etc_common
      condition: >
        ...
        and not proc.name in (passwd_binaries, shadowutils_binaries, sysdigcloud_binaries,
                              ...
                              openshift-launc, update-rc.d, puppet, diginfractl)
    ...
    # older local diginfra rules files that may have been appending to
    # trusted_images. To make customizations, it's better to add images to
    # either privileged_images or diginfra_sensitive_mount_images.
    - list: trusted_images
      items: []
    ...
    # Diginfra containers
    - list: diginfra_containers
      items:
        - diginfra/diginfra
        - docker.io/diginfra/diginfra
        - public.ecr.aws/diginfra/diginfra
    
    # Diginfra no driver containers
    - list: diginfra_no_driver_containers
      items:
        - diginfra/diginfra-no-driver
        - docker.io/diginfra/diginfra-no-driver
        - public.ecr.aws/diginfra/diginfra-no-driver
    ...
    - list: diginfra_privileged_images
      items: [
        diginfra_containers,
    ...
    - macro: diginfra_privileged_containers
      condition: (openshift_image or
                  user_trusted_containers or
                  aws_eks_core_images or
                  container.image.repository in (trusted_images) or
                  container.image.repository in (diginfra_privileged_images) or
    ...
    #
    # In this file, it just takes one of the images in diginfra_privileged_images
    # and repeats it.
    ...
    # host filesystem.
    - list: diginfra_sensitive_mount_images
      items: [
        diginfra_containers,
    ...
    - macro: diginfra_sensitive_mount_containers
      condition: (user_trusted_containers or
                  aws_eks_image_sensitive_mount or
                  container.image.repository in (trusted_images) or
                  container.image.repository in (diginfra_sensitive_mount_images) or
    ...
    #
    # In this file, it just takes one of the images in diginfra_sensitive_mount_images
    # and repeats it.
    ...
    - rule: Launch Privileged Container
      ...
      condition: >
        ...
        and not diginfra_privileged_containers
    ...
    - rule: Launch Sensitive Mount Container
      ...
      condition: >
        container_started and container
        and sensitive_mount
        and not diginfra_sensitive_mount_containers
    ...
    # your environment. In this main diginfra rules file, there isn't any way
    ...
    # https://github.com/diginfra/diginfra/issues/255 is fixed this will be a
    ...
    # With the current restriction on system calls handled by diginfra
    ...
    # access to EC2 metadata. In this main diginfra rules file, there isn't
    ...
    - macro: trusted_images_query_miner_domain_dns
      condition: (container.image.repository in (diginfra_containers))
    
    # The rule is disabled by default.
    # Note: diginfra will send DNS request to resolve miner pool domain which may trigger alerts in your environment.
    ...
    # ****************************************************************************
    # * "Container Drift Detected (open+create)" requires DIGINFRA_ENGINE_VERSION 6 *
    # ****************************************************************************
    ...
    # Use Diginfra HELM chart to update (append) the c2 server lists with your values.
    # See an example below.
    #
    #  `+"`"+``+"`"+``+"`"+`yaml
    #  # values.yaml Diginfra HELM chart file
    #  [...]
    #  customRules:
    ...
    # init_module and finit_module syscalls are available since Diginfra 0.35.0
    ...
    - rule: Execution from /dev/shm
      ...
      condition: >
        ...
        not container.image.repository in (diginfra_privileged_images, trusted_images)
