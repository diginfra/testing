...
var FileOutput = run.NewStringFileAccessor(
	"file_output.yaml",
	`
# File containing Diginfra rules, loaded at startup.
rules_file: /etc/diginfra_rules.yaml

# Whether to output events in json or text
json_output: false

# Send information logs to stderr and/or syslog Note these are *not* security
# notification logs! These are just Diginfra lifecycle (and possibly error) logs.
log_stderr: false
log_syslog: false
...
file_output:
  enabled: true
  filename: /tmp/diginfra_outputs/file_output.txt
...
program_output:
  enabled: false
  program: mail -s "Diginfra Notification" someone@example.com
`,
)
...
# These are just Diginfra lifecycle (and possibly error) logs.
log_stderr: false
log_syslog: false
...
grpc:
    enabled: true
    bind_address: "unix:///tmp/diginfra/diginfra.sock"
    threadiness: 8
...
program_output:
  enabled: true
  program: cat >> /tmp/diginfra_outputs/program_output.txt
`,
)
