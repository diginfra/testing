...
// see tests/diginfra/legacy_test.go for more details.
...
func runDiginfraWithK8SAudit(t *testing.T, r run.Runner, input run.FileAccessor, opts ...diginfra.TestOption) *diginfra.TestOutput {
	...
	options := []diginfra.TestOption{
		diginfra.WithEnabledSources("k8s_audit"),
		diginfra.WithConfig(config),
		diginfra.WithExtraFiles(input, plugins.K8SAuditPlugin, plugins.JSONPlugin),
	}
...
func TestK8SAudit_Legacy_JsonPointerCorrectParse(t *testing.T) {
	...
	res := runDiginfraWithK8SAudit(t,
		...
		captures.K8SAuditCreateNginxPodUnprivileged,
		diginfra.WithOutputJSON(),
		diginfra.WithRules(rules.K8SAuditSingleRuleWithJsonPointer),
	)
	assert.NotZero(t, res.Detections().Count())
	assert.NotZero(t, res.Detections().OfPriority("WARNING").Count())
