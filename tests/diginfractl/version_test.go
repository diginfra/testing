...
func TestDiginfractl_Version(t *testing.T) {
	t.Parallel()
	...
	t.Run("invalid-extra-cmd", func(t *testing.T) {
		...
		res := diginfractl.Test(
			...
			diginfractl.WithArgs("version", "some_other_cmd"),
		)
	...
	t.Run("version-json", func(t *testing.T) {
		...
		res := diginfractl.Test(
			tests.NewDiginfractlExecutableRunner(t),
			diginfractl.WithArgs("version", "--output=json"),
		)
		assert.NoError(t, res.Err(), "%s", res.Stderr())
	...
	t.Run("version-yaml", func(t *testing.T) {
		...
		res := diginfractl.Test(
			tests.NewDiginfractlExecutableRunner(t),
			diginfractl.WithArgs("version", "--output=yaml"),
		)
		assert.NoError(t, res.Err(), "%s", res.Stderr())
		assert.Equal(t, res.ExitCode(), 0)
		out := make(map[string]interface{})
