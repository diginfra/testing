...
// WithArgs runs diginfractl with the given arguments.
func WithArgs(args ...string) TestOption {
	return func(ro *testOptions) { ro.args = append(ro.args, args...) }
}

// WithConfig runs diginfractl with the given config file through the `--config` option.
...
// WithPluginsDir runs diginfractl with the given custom plugins dir file through the `--plugins-dir` option.
...
// WithRulesFilesDir runs diginfractl with the given custom rules files dir file through the `--rulesfiles-dir` option.
