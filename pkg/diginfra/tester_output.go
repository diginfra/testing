...
// Err returns a non-nil error in case of issues when running Diginfra.
func (t *TestOutput) Err() error {
	return multierr.Append(t.opts.err, t.err)
}

// DurationExceeded returns true if the Diginfra run exceeded the expected
...
// ExitCode returns the numeric exit code of the Diginfra process.
...
// Stdout returns a string containing the stdout output of the Diginfra run.
func (t *TestOutput) Stdout() string {
	return t.stdout.String()
}

// Stderr returns a string containing the stderr output of the Diginfra run.
func (t *TestOutput) Stderr() string {
	return t.stderr.String()
}

// StdoutJSON deserializes the stdout of the Diginfra run using the JSON encoding.
