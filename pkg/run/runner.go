...
type Runner interface {
	// Run runs Diginfra with the given options and returns when it finishes its
...
// WithFiles is an option for running Diginfra with some files
// to be used during execution and/or referenced in the CLI args
// (e.g. rules files, config files, capture files, etc...).
// For example, if you run Diginfra with `-c` (through WithArgs), you
...
// WithEnvVars is an option for running Diginfra with a given set of
